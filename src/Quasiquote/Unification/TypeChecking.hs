...
module Quasiquote.Unification.TypeChecking where
...
import Quasiquote.Core.Evaluation ()
...
type Context = [(Int,String,Term,QuoteLevel)]
...
runTypeChecker checker sig defs ctx i als mods
  = runStateT checker (TCState sig defs ctx i 0 [] als mods 0)
...
quoteLevel :: TypeChecker Int
quoteLevel = fmap tcQuoteLevel get

withQuoteLevel :: Int -> TypeChecker a -> TypeChecker a
withQuoteLevel l tc = do s <- get
                         put (s { tcQuoteLevel = l })
                         x <- tc
                         s' <- get
                         put (s' { tcQuoteLevel = tcQuoteLevel s })
                         return x
...
occurs x (Quoted a)         = occurs x a
occurs x (Quote m)          = occurs x m
occurs x (Unquote m sc)     = occurs x m || occurs x (descope (Var . Name) sc)
...
solve eqs0 = go eqs0 []
  where
    ...
    go (Equation (Quoted a1) (Quoted a2):eqs) subs'
      = go (Equation a1 a2:eqs) subs'
    go (Equation (Quote m1) (Quote m2):eqs) subs'
      = go (Equation m1 m2:eqs) subs'
    go (Equation (Unquote m1 sc1) (Unquote m2 sc2):eqs) subs'
      = do i <- newName
           let b1 = instantiate sc1 [Var (Generated (head (names sc1)) i)]
               b2 = instantiate sc2 [Var (Generated (head (names sc2)) i)]
           go (Equation m1 m2 : Equation b1 b2 : eqs) subs'
...
addSubstitutions subs0
  ...
  where
    ...
    substituteContext
      = do ctx <- context
           subs <- substitution
           putContext (map (\(i,x,t,l) -> (i, x, instantiateMetas subs t,l)) ctx)
...
instantiateMetas subs (Quoted a)
  = Quoted (instantiateMetas subs a)
instantiateMetas subs (Quote m)
  = Quote (instantiateMetas subs m)
instantiateMetas subs (Unquote m sc)
  = Unquote (instantiateMetas subs m)
            (instantiateMetas subs <$> sc)
...
typeInContext :: Int -> TypeChecker (Term,Int)
typeInContext i
  = do ctx <- context
       case find (\(j,_,_,_) -> j == i) ctx of
         Nothing      -> throwError "Unbound automatically generated variable."
         Just (_,_,t,l) -> return (t,l)
...
inferify (Var (Generated x i))
  = do (t,l) <- typeInContext i
       l' <- quoteLevel
       unless (l == l')
         $ throwError $ "Mismatching quote levels for " ++ show (Var (Generated x i))
...
inferify (Fun plic arg sc)
  = do arg' <- checkify arg Type
       i <- newName
       l <- quoteLevel
       ret' <- extendContext [(i, head (names sc), arg', l)]
  ...
  where
    ...
    checkifyTelescope (TelescopeCons t sc)
      = do t' <- checkify t Type
           i <- newName
           l <- quoteLevel
           m' <- extendContext [(i, head (names sc), t', l)]
...
inferify (Quote m)
  = do l <- quoteLevel
       (m',t) <- withQuoteLevel (l+1)
                   $ infer m
       return (Quote m',Quoted t)
inferify (Unquote m sc)
  = do (m',t) <- inferify m
       et <- evaluate t
       case et of
         Quoted t'
           -> do i <- newName
                 l <- quoteLevel
                 (b',t'')
                   <- extendContext [(i, head (names sc), t', l+1)]
                        $ inferify (instantiate sc [Var (Generated (head (names sc)) i)])
                 return (Unquote m' (Scope (names sc) (abstractOver [i] b')), t'')
         _ -> throwError $ "The argument of unquote " ++ show m ++ " must be of type Quoted "
                        ++ "but instead has the type " ++ show t
       ...
       case (plic,et) of
         (Expl, Fun Expl arg sc') -> -- \x -> M : (x : A) -> B
           do i <- newName
              ...
              l <- quoteLevel
              m' <- extendContext [(i, head (names sc), arg, l)]
                      $ checkify
         ...
         (Expl, Fun Impl arg sc') -> -- \x -> M : {y : A} -> B
           do i <- newName
              eret <- evaluate (instantiate sc' [Var (Generated (head (names sc)) i)])
              l <- quoteLevel
              f' <- extendContext [(i, head (names sc), arg, l)]
                      $ checkify
                          (Lam Expl sc)
                          eret
              subs <- substitution
...
checkify (Quote m) t
  = do et <- evaluate t
       case et of
         Quoted a
           -> do l <- quoteLevel
                 m' <- withQuoteLevel (l+1)
                         $ checkify m a
                 return $ Quote m'
         _ -> throwError $ "Cannot check term: " ++ show (Quote m)
                        ++ "\nAgainst non-Quoted type: " ++ show t
checkify m t
  = do (m',t') <- inferify m
       et <- evaluate t
       et' <- evaluate t'
...
checkifyCaseMotive (CaseMotiveCons a sc)
  = do a' <- checkify a Type
       i <- newName
       l <- quoteLevel
       b' <- extendContext [(i, head (names sc), a', l)]
...
checkifyPattern (VarPat (Generated x i)) t
  = do (t',l') <- typeInContext i
       l <- quoteLevel
       unless (l == l')
         $ throwError $ "Mismatching quote levels for pattern " ++ show (VarPat (Generated x i))
       unify t t'
...
checkifyConSig (ConSigCons plic arg sc)
  = do arg' <- checkify arg Type
       i <- newName
       l <- quoteLevel
       t <- extendContext [(i, head (names sc), arg', l)]
