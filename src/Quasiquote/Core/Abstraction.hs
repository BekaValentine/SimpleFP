...
module Quasiquote.Core.Abstraction where
...
instance Abstract String Term Term where
  ...
  abstract (Quote m)
    = Quote <$> abstract m
  abstract (Unquote m sc)
    = Unquote <$> abstract m <*> abstractScope sc
...
unquoteHelper :: Term -> String -> Term -> Term
unquoteHelper m x b = Unquote m (scope [x] b)