...
inferify (Con c as)
  = do consig <- typeInSignature c
       ...
       subs <- substitution
       return (instantiateMetas subs (Con c as'), instantiateMetas subs ret)
  where
...
checkify (Con c as) t
  = do consig <- typeInSignature c
       ...
       subs <- substitution
       return (instantiateMetas subs (Con c as'))
  where
...
checkifyClause (Clause ps sc0) motive
  = do (ps',ctx,ret) <- checkPatternSeqMotive ps motive
       ...
       return $ Clause ps' (Scope (names sc0) (abstractOver is m'))
  where
